...
Like HTML, CSS is not really a programming language. It is not a _markup language_ either — it is a _style sheet language_. This means that it lets you apply styles selectively to elements in HTML documents. For example, to select **all** the paragraph elements on an HTML page and turn the text within them red, you'd write this CSS:
...
![A mozilla logo and some paragraphs. The paragraph text has been styled red by our css.](https://mdn.mozillademos.org/files/9435/website-screenshot-styled.png)If your paragraph text is now red, congratulations! You've just written your first successful CSS.
...
Ways in which you can style a given HTML element. (In this case, `color` is a property of the {{htmlelement("p")}} elements.) In CSS, you choose which properties you want to affect in your rule.
